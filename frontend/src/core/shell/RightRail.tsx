...
export default function RightRail() {
  const { entries, activeModuleId, togglePanel } = useRightPanelStore()
  ...
  return (
    <Tooltip.Provider delayDuration={300}>
      ...
      >
        {entries.map(({ moduleId, icon: Icon, label }) => {
            ...
            <Tooltip.Root key={moduleId}>
              ...
              <Tooltip.Portal>
                <Tooltip.Content
                  side="left"
                  sideOffset={8}
                  className="px-2.5 py-1.5 text-xs rounded-md shadow-md select-none z-50"
